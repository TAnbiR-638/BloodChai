...
?>
    <div class="container"  style='margin-top:70px;'>
		...
		$sql = "INSERT INTO request_blood (NAME, GENDER, BLOOD, HOSP, CITY, AREA, DOC, RDATE, CNAME, EMAIL, CON1, CON2, REASON, PIC, CADDRESS)
        VALUES('{$_POST["NAME"]}', '{$_POST["GENDER"]}', '{$_POST["BLOOD"]}', '{$_POST["HOSP"]}', '{$_POST["CITY"]}', '{$_POST["AREA"]}', '{$_POST["DOC"]}', '{$_POST["RDATE"]}', '{$_POST["CNAME"]}', '{$_POST["EMAIL"]}', '{$_POST["CON1"]}', '{$_POST["CON2"]}', '{$_POST["REASON"]}', '{$target_file}', '{$_POST["CADDRESS"]}')";
			...
			if ($con->query($sql)) {
				...
				echo "<div class='alert alert-danger fade in'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Error : </strong>Server busy. Try again later.</div>";
					...
					<form autocomplete="off" method="post"  action="<?php echo $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data">
						...
						<div class="form-group">
									<label class="control-label text-primary" for="COUNTRY">City</label>
...
<script>
	$(document).ready(
		...
		if($("#BLOOD").val() == "" )
				...
				{
					$("#AREA").focus();
					$("#errorBox").html("<div class='alert alert-danger fade in' ><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Warning : </strong> Please Enter Your area name correctly.</div>");
					return false;
				}
